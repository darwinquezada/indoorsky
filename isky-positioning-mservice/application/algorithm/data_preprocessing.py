...
def data_reshape_sample_timestep(data):
    if np.shape(data)[1] % 2 != 0:
        data = np.append(data, np.zeros((np.shape(data)[0], 1)), axis=1)
    data_series = data.reshape((data.shape[0], data.shape[1], 1)) 
    return data_series

def data_reshape_sample_timestep_feature(data):
    data_series = data_reshape_sample_timestep(data)
    subsequences = 2
