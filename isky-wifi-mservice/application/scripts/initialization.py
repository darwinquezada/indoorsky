...
def verify_table(db_conn) -> dict:
    try:
        ...
        if not list_indexes :
            r.db(DATABASE).table(TABLE).index_create("fingerprint_id").run(db_conn)
            r.db(DATABASE).table(TABLE).index_wait("fingerprint_id").run(db_conn)
            r.db(DATABASE).table(TABLE).index_create("ssid").run(db_conn)
            r.db(DATABASE).table(TABLE).index_wait("ssid").run(db_conn)
            r.db(DATABASE).table(TABLE).index_create("bssid").run(db_conn)
            r.db(DATABASE).table(TABLE).index_wait("bssid").run(db_conn)
            pass
