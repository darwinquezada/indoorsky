...
def verify_table(db_conn) -> dict:
    try:
        ...
        if not list_indexes :
            r.db(DATABASE).table(TABLE).index_create("device_id").run(db_conn)
            r.db(DATABASE).table(TABLE).index_wait("device_id").run(db_conn)
            r.db(DATABASE).table(TABLE).index_create("name").run(db_conn)
            r.db(DATABASE).table(TABLE).index_wait("name").run(db_conn)
            pass
